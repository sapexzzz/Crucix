...
export async function briefing() {
  ...
  const signals = [];
  const loc = getLocale();
  const lang = loc?.meta?.code || 'en';
  for (const h of hotspots) {
    if (h.highIntensity?.length > 5) {
      signals.push(lang === 'ru'
        ? `ИНТЕНСИВНЫЕ ПОЖАРЫ в ${h.region}: ${h.highIntensity.length} обнаружений >10МВт ТМИ`
        : `HIGH INTENSITY FIRES in ${h.region}: ${h.highIntensity.length} detections >10MW FRP`
      );
    }
    if (h.nightDetections > 20) {
      signals.push(lang === 'ru'
        ? `ПОВЫШЕННАЯ НОЧНАЯ АКТИВНОСТЬ в ${h.region}: ${h.nightDetections} ночных обнаружений (возможные удары/бои)`
