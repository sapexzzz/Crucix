...
export class TelegramAlerter {
  constructor({ botToken, chatId }) {
    ...
    // Support comma-separated chat IDs: "123,456" → ['123', '456']
    this.chatIds = chatId
      ? String(chatId).split(',').map(id => id.trim()).filter(Boolean)
      : [];
    this.chatId = this.chatIds[0] ?? null; // primary ID (backward compat)
  ...
  async sendMessage(message, opts = {}) {
    if (!this.isConfigured) {
      console.warn('[Telegram] sendMessage skipped: not configured (botToken or chatIds missing)');
      return { ok: false };
    }
    // If a specific chatId is given (e.g. command reply), send only to that one.
    // Otherwise broadcast to all configured chat IDs.
    const targets = opts.chatId ? [opts.chatId] : this.chatIds;
    const parseMode = opts.parseMode || 'Markdown';
    const chunks = this._chunkText(message, TELEGRAM_MAX_TEXT);
    console.log(`[Telegram] Sending to ${targets.length} chat(s): ${targets.join(', ')} | chunks: ${chunks.length}`);
      ...
      try {
        for (let i = 0; i < chunks.length; i++) {
          ...
          if (!res.ok) {
            ...
            console.error(`[Telegram] Send failed to ${chatId} (${res.status}): ${err.substring(0, 200)}`);
            break;
          }
  ...
  _ruleBasedEvaluation(signals, delta) {
    ...
    if (criticals.length >= 2 && hasCriticalMarket && hasCriticalConflict) {
      return {
        shouldAlert: true, tier: 'FLASH', confidence: 'HIGH',
        headline: (rh.crossDomainSignals || '{count} Critical Cross-Domain Signals').replace('{count}', criticals.length),
    ...
    if (escalatedHighs.length >= 2) {
      return {
        ...
        headline: (rh.escalatingSignals || '{count} Escalating Signals').replace('{count}', escalatedHighs.length),
    ...
    if (osintNew.length >= 5) {
      return {
        shouldAlert: true, tier: 'PRIORITY', confidence: 'MEDIUM',
        headline: (rh.osintSurge || 'OSINT Surge: {count} New Urgent Posts').replace('{count}', osintNew.length),
    ...
    if (criticals.length >= 1 || highs.length >= 3) {
      ...
      return {
        ...
        headline: topSignal.label || topSignal.reason || (rh.signalChangeDetected || 'Signal Change Detected'),
  ...
  async _pollUpdates() {
    try {
      ...
      for (const update of data.result) {
        ...
        // Restrict command execution to configured chat IDs only.
  ...
  async _handleMessage(msg) {
    ...
    if (command === '/help') {
      const loc = getLocale();
      const bc = loc.bot?.commands || {};
      const tip = loc.bot?.messages?.commandsTip || 'Tip: Commands are case-insensitive';
      const helpText = Object.entries(COMMANDS)
        .map(([cmd, desc]) => `${cmd} — ${bc[cmd.slice(1)] || desc}`)
        .join('\n');
    ...
    if (command === '/mute') {
      ...
      const bm = getLocale().bot?.messages || {};
      const muteMsg = (bm.alertsMuted || '🔇 Alerts muted for {hours}h — until {time} UTC')
        .replace('{hours}', hours)
        .replace('{time}', new Date(this._muteUntil).toLocaleTimeString());
      const unmuteHint = bm.useUnmute || 'Use /unmute to resume.';
      await this.sendMessage(
        `${muteMsg}\n${unmuteHint}`,
        { chatId: replyChatId, replyToMessageId: msg.message_id }
      );
    ...
    if (command === '/unmute') {
      ...
      const bm = getLocale().bot?.messages || {};
      await this.sendMessage(
        bm.alertsResumed || '🔔 Alerts resumed. You\'ll receive the next signal evaluation.',
        { chatId: replyChatId, replyToMessageId: msg.message_id }
      );
      return;
    }
    ...
    if (command === '/alerts') {
      ...
      const bm = getLocale().bot?.messages || {};
      if (recent.length === 0) {
        await this.sendMessage(bm.noRecentAlerts || 'No recent alerts.', { chatId: replyChatId, replyToMessageId: msg.message_id });
        return;
      }
      const lines = recent.map(a =>
        `${TIER_CONFIG[a.tier]?.emoji || '⚪'} ${a.tier} — ${new Date(a.timestamp).toLocaleTimeString()}`
      );
      const header = (bm.recentAlerts || '📋 Recent Alerts (last {count})').replace('{count}', recent.length);
      await this.sendMessage(
        `${header}\n\n${lines.join('\n')}`,
        { chatId: replyChatId, replyToMessageId: msg.message_id }
      );
    ...
    if (handler) {
      try {
        ...
        const bm = getLocale().bot?.messages || {};
        await this.sendMessage(
          (bm.commandFailed || '❌ Command failed: {error}').replace('{error}', err.message),
          { chatId: replyChatId, replyToMessageId: msg.message_id }
        );
      }
    }
...
Respond with ONLY valid JSON:
{
  ...
  _formatTieredAlert(evaluation, delta, tier) {
    ...
    const loc = getLocale();
    const af = loc.alerts?.fields || {};
    const confLabels = loc.alerts?.confidence || {};
    const tierLabel = loc.bot?.alertTiers?.[tier.toLowerCase()] || tc.label;
    const confidenceEmoji = { HIGH: '🟢', MEDIUM: '🟡', LOW: '⚪' }[evaluation.confidence] || '⚪';
    const confLabel = confLabels[(evaluation.confidence || 'medium').toLowerCase()] || evaluation.confidence || 'MEDIUM';

    const lines = [
      `${tc.emoji} *CRUCIX ${tierLabel}*`,
      ``,
      `*${evaluation.headline}*`,
      ``,
      evaluation.reason,
      ``,
      `${af.confidence || 'Confidence'}: ${confidenceEmoji} ${confLabel}`,
      `${af.direction || 'Direction'}: ${delta.summary.direction.toUpperCase()}`,
    ];

    if (evaluation.crossCorrelation) {
      lines.push(`${af.crossCorrelation || 'Cross-correlation'}: ${evaluation.crossCorrelation}`);
    }

    if (evaluation.actionable && evaluation.actionable !== 'Monitor' && evaluation.actionable !== (af.monitor || 'Monitor')) {
      lines.push(``, `💡 *${af.action || 'Action'}:* ${evaluation.actionable}`);
    }

    if (evaluation.signals?.length) {
      lines.push('', `*${af.signals || 'Signals'}:*`);
