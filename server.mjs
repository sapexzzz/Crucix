...
if (telegramAlerter.isConfigured) {
  ...
  telegramAlerter.onCommand('/status', async () => {
    ...
    const loc = getLocale();
    const bs = loc.bot?.status || {};
    const llmStatus = llmProvider?.isConfigured
      ? `${bs.enabled || '✅'} ${llmProvider.name}`
    ...
    return [
      `🖥️ *${bs.title || 'CRUCIX STATUS'}*`,
      ``,
      `${bs.uptime || 'Uptime'}: ${h}h ${m}m`,
      `${bs.lastSweep || 'Last sweep'}: ${lastSweepTime ? new Date(lastSweepTime).toLocaleTimeString() + ' UTC' : (bs.never || 'never')}`,
      `${bs.nextSweep || 'Next sweep'}: ${nextSweep} UTC`,
      `${bs.sweepInProgress || 'Sweep in progress'}: ${sweepInProgress ? (bs.yes || '🔄 Yes') : (bs.no || '⏸️ No')}`,
      `${bs.sources || 'Sources'}: ${sourcesOk}/${sourcesTotal} OK${sourcesFailed > 0 ? ` (${sourcesFailed} ${bs.failed || 'failed'})` : ''}`,
      `${bs.llm || 'LLM'}: ${llmStatus}`,
      `${bs.sseClients || 'SSE clients'}: ${sseClients.size}`,
      `${bs.dashboard || 'Dashboard'}: http://localhost:${config.port}`,
    ].join('\n');
  });

  telegramAlerter.onCommand('/sweep', async () => {
    const bm = getLocale().bot?.messages || {};
    if (sweepInProgress) return bm.sweepInProgress || '🔄 Sweep already in progress. Please wait.';
    // Fire and forget — don't block the bot response
    runSweepCycle().catch(err => console.error('[Crucix] Manual sweep failed:', err.message));
    return bm.sweepTriggered || '🚀 Manual sweep triggered. You\'ll receive alerts if anything significant is detected.';
  });

  telegramAlerter.onCommand('/brief', async () => {
    const loc = getLocale();
    const bm = loc.bot?.messages || {};
    const bb = loc.bot?.brief || {};
    if (!currentData) return bm.noDataYet || '⏳ No data yet — waiting for first sweep to complete.';
    ...
    if (delta?.summary) {
      ...
      sections.push(`${dirEmoji} ${bb.direction || 'Direction'}: *${delta.summary.direction.toUpperCase()}* | ${delta.summary.totalChanges} ${bb.changes || 'changes'}, ${delta.summary.criticalChanges} ${bb.criticalChanges || 'critical'}`);
      sections.push('');
    }
    ...
    if (tg.urgent?.length > 0) {
      sections.push(`${bb.osint || '📡 OSINT'}: ${tg.urgent.length} ${bb.urgentSignals || 'urgent signals'}, ${tg.posts || 0} ${bb.totalPosts || 'total posts'}`);
    ...
    // Top ideas
    if (ideas.length > 0) {
      sections.push(`${bb.topIdeas || '💡 *Top Ideas:*'}`);
  ...
  telegramAlerter.onCommand('/portfolio', async () => {
    const bm = getLocale().bot?.messages || {};
    return bm.portfolioNotAvailable || '📊 Portfolio integration requires Alpaca MCP connection.\nUse the Crucix dashboard or Claude agent for portfolio queries.';
  });
...
if (discordAlerter.isConfigured) {
  ...
  discordAlerter.onCommand('brief', async () => {
    ...
    const sections = [`**${bb.title || '📋 CRUCIX BRIEF'}**\n_${new Date().toISOString().replace('T', ' ').substring(0, 19)} UTC_\n`];
...
app.get('/', (req, res) => {
  if (!currentData) {
    ...
    // Inject current in-memory data so every fresh page load shows up-to-date values,
    // regardless of what snapshot is baked into jarvis.html on disk.
    const dataJson = JSON.stringify(currentData).replace(/<\/script>/gi, '<\\/script>');
    html = html.replace(/^(let|const) D = .*;\s*$/m, () => 'let D = ' + dataJson + ';');
