...
> - All dashboard panels are now fully translated when `CRUCIX_LANG=ru`: panel headers, badges, map legend, nuclear/risk/space/signal metrics, OSINT timestamps, delta change badges, trade ideas, and disclosures
> - Telegram bot commands (`/status`, `/sweep`, `/brief`, `/portfolio`, `/alerts`, `/help`, `/mute`, `/unmute`) and all alert messages are sent in the configured language
> - Alert formatting (`_formatTieredAlert`, rule-based headlines) and FIRMS signal strings are locale-aware
>
> **Bug fixes**
> - Fixed bugs present in the original codebase (3 april bug, no russian locale...)
